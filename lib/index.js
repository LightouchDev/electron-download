...
class ElectronDownloader {
  ...
  get chromedriverVersion () {
    return process.env.npm_config_chromedriver_version || '2.33'
  }
  ...
  get filename () {
    ...
    if (this.chromedriver) {
      return `chromedriver-v${this.chromedriverVersion}-${type}.zip`
    } else if (this.mksnapshot) {
      return `mksnapshot-${suffix}.zip`
    } else if (this.ffmpeg) {
      return `ffmpeg-${suffix}.zip`
    } else if (this.symbols) {
      return `brave-${suffix}-symbols.zip`
    } else if (this.dsym) {
      return `brave-${suffix}-dsym.zip`
    } else {
      return `brave-${suffix}.zip`
    }
  }
  ...
  get version () {
    return process.env.npm_config_brave_electron_version || this.opts.version
  }
